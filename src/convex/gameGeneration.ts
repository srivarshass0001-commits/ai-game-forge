...
function generatePuzzleGame(prompt: string, parameters: any) {
  ...
  // NEW: Detect Memory/Match/Pairs/Concentration and route to Memory game
...
// NEW: Dedicated Memory Match generator with colorful cards and flip animations
function generateMemoryGame(prompt: string, parameters: any) {
  const tuning = analyzePrompt(prompt, parameters);
  // Choose a grid size suitable for most prompts
  const rows = 4;
  const cols = 4; // 8 pairs
  const totalPairs = (rows * cols) / 2;
  const previewMs = 3000; // brief reveal
  const backColor = 0x2d2f36;

  // A set of bright colors to assign to pairs, influenced by theme color
  const baseColors = [
    tuning.mainColor,
...
class MemoryGame extends Phaser.Scene {
  constructor() {
    ...
    this.offsetY = 80;
    this.firstPick = null;
    this.locked = true; // locked during preview
    this.matchedPairs = 0;
    this.moves = 0;
    this.cards = [];
    this.frontTextures = [];
  ...
  preload() {
    ...
    // Generate colorful fronts for each pair with a simple icon/label
    ...
    for (let i = 0; i < ${totalPairs}; i++) {
      ...
      // Add a simple icon: circle and a letter/number for variety
  ...
  create() {
    // Build deck: pair each index twice and shuffle
    const deck = [];
    for (let i = 0; i < ${totalPairs}; i++) {
      deck.push(i, i);
    }
    ...
    for (let r = 0; r < this.rows; r++) {
      for (let c = 0; c < this.cols; c++) {
        ...
        // Create container to allow flip via scaleX tween
        ...
        container.setData('showingFront', true); // start face-up for preview
    ...
    // Show preview then flip all down with animation
  ...
  onCardClick(cardContainer) {
    ...
    if (cardContainer.getData('showingFront')) return; // already face-up during the round
    ...
    this.flipCard(cardContainer, true, () => {
      if (!this.firstPick) {
        ...
        if (a === b) {
          ...
          // Not a match: flip both down after short delay
  ...
  getCardSprites(container) {
    // find front/back by checking children visibility
  ...
  endGame() {
    const base = 1000;
    const penalty = Math.max(0, (this.moves - ${totalPairs}) * 12); // fewer moves => higher score
